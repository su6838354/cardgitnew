...
var	imgpath = '../../static/assets/images/mobile/';
var misc = {
	/**
	* 获取URL参数
	* @method getParam
...
misc.api.user.update_user_checkin=misc.api.base+'update_user_checkin';
...
misc.func.user.update_user_checkin=function(param,succ_cb,fail_cb){
	misc.ajax.cdPost(misc.api.user.update_user_checkin, JSON.stringify(param), function(res){
        succ_cb && succ_cb(res);
    }, function(err){
        fail_cb && fail_cb(err);
    });
};
misc.func.activity.get_activities_by_join=function(param,succ_cb,fail_cb){
    misc.ajax.cdPost(misc.api.activity.get_activities_by_join, JSON.stringify(param), function(res){
        succ_cb && succ_cb(res);
    }, function(err){
        fail_cb && fail_cb(err);
    });
...
misc.fillAlert = function($ele,s,time){
	$ele.text(s).show();
	time= time||600;
	setTimeout(function(){
		$ele.hide();
...
function loading(){
	$('body').append(['<img class="loading" id="loading" src="',imgpath,'loading.gif" />'].join(''));
}
function loadingHide(){
	var loadingobj = $('#loading');
	loadingobj && loadingobj.remove();
}
function menuHtml(img1,img2){
  var $page_nav = $('#page-nav');
  $page_nav.html(['<li data-url="../index/">',
                    '<div class="img-div">',
                      '<img class="menu-icon-img" src="',imgpath,'icon/',img1,'.png" />',
                    '</div>',
                    '<div class="menu-icon-name">商户</div>',
                  '</li>',
                  '<li data-url="../me/">',
                    '<div class="img-div">',
                      '<img class="menu-icon-img" src="',imgpath,'icon/',img2,'.png" />',
                    '</div>',
                    '<div class="menu-icon-name">我的</div>',
                  '</li>'].join(''));

  $page_nav.delegate('li', 'click', function(e) {
      e.preventDefault();
      e.stopPropagation();
      window.location.href = $(this).attr('data-url');
  });
...
user.logIn=misc.func._user.login;
user.logOut=function(){
    misc.clearCookie('userInfo');
	redirect(undefined,0);
};
