...
/* echo.min.js */
(function(root,factory){if(typeof define==="function"&&define.amd){define(function(){return factory(root)})}else{if(typeof exports==="object"){module.exports=factory}else{root.echo=factory(root)}}})(this,function(root){var echo={};var callback=function(){};var offset,poll,delay,useDebounce,unload;var isHidden=function(element){return(element.offsetParent===null)};var inView=function(element,view){if(isHidden(element)){return false}var box=element.getBoundingClientRect();return(box.right>=view.l&&box.bottom>=view.t&&box.left<=view.r&&box.top<=view.b)};var debounceOrThrottle=function(){if(!useDebounce&&!!poll){return}clearTimeout(poll);poll=setTimeout(function(){echo.render();poll=null},delay)};echo.doImage=function(imgUrl,index,loadSucc){var newImg=new Image();newImg.onload=loadSucc;newImg.src=imgUrl;newImg.alt=index};echo.init=function(opts){opts=opts||{};var offsetAll=opts.offset||0;var offsetVertical=opts.offsetVertical||offsetAll;var offsetHorizontal=opts.offsetHorizontal||offsetAll;var optionToInt=function(opt,fallback){return parseInt(opt||fallback,10)};offset={t:optionToInt(opts.offsetTop,offsetVertical),b:optionToInt(opts.offsetBottom,offsetVertical),l:optionToInt(opts.offsetLeft,offsetHorizontal),r:optionToInt(opts.offsetRight,offsetHorizontal)};delay=optionToInt(opts.throttle,250);useDebounce=opts.debounce!==false;unload=!!opts.unload;callback=opts.callback||callback;echo.render();if(document.addEventListener){root.addEventListener("scroll",debounceOrThrottle,false);root.addEventListener("load",debounceOrThrottle,false)}else{root.attachEvent("onscroll",debounceOrThrottle);root.attachEvent("onload",debounceOrThrottle)}};echo.render=function(){var nodes=document.querySelectorAll("img[data-echo], [data-echo-background]");var length=nodes.length;var src,elem;var view={l:0-offset.l,t:0-offset.t,b:(root.innerHeight||document.documentElement.clientHeight)+offset.b,r:(root.innerWidth||document.documentElement.clientWidth)+offset.r};for(var i=0;i<length;i++){elem=nodes[i];if(inView(elem,view)){if(unload){elem.setAttribute("data-echo-placeholder",elem.src)}if(elem.getAttribute("data-echo-background")!==null){echo.doImage(elem.getAttribute("data-echo-background"),i,function(data){nodes[parseInt(data.srcElement.alt)].style.backgroundImage="url("+data.srcElement.src+")"})}else{echo.doImage(elem.getAttribute("data-echo"),i,function(data){nodes[parseInt(data.srcElement.alt)].src=data.srcElement.src})}if(!unload){elem.removeAttribute("data-echo");elem.removeAttribute("data-echo-background")}callback(elem,"load")}else{if(unload&&!!(src=elem.getAttribute("data-echo-placeholder"))){if(elem.getAttribute("data-echo-background")!==null){elem.style.backgroundImage="url("+src+")"}else{elem.src=src}elem.removeAttribute("data-echo-placeholder");callback(elem,"unload")}}}if(!length){echo.detach()}};echo.detach=function(){if(document.removeEventListener){root.removeEventListener("scroll",debounceOrThrottle)}else{root.detachEvent("onscroll",debounceOrThrottle)}clearTimeout(poll)};return echo});
...
showPages.prototype.getPage = function(){ //从url获得当前页的参数,如果变量重复只获取最后一个
 ...
 switch (mode) {
  case 1 : //模式1 (10页缩略,首页,前页,后页,尾页)
   ...
   if (prevPage < 1) {
    ...
    strHtml += '<span title="First Page" onclick="javascript:' + this.name + '.toPage(1);"><a href="javascript:;">首页</a></span>';
    strHtml += '<span title="Prev Page" onclick="javascript:' + this.name + '.toPage(' + prevPage + ');"><a href="javascript:;">上一页</a></span>';
   }
   if (this.page % 10 ==0) {
   ...
   if (startPage > 10) strHtml += '<span title="Prev 10 Pages" onclick="javascript:' + this.name + '.toPage(' + (startPage - 1) + ');"><a href="javascript:;">...</a></span>';
   for (var i = startPage; i < startPage + 10; i++) {
    if (i > this.pageCount) break;
    if (i == this.page) {
     strHtml += '<span title="Page ' + i + ' " style="color:#fff;background-color: #666 !important;" >' + i + '</span>';
    } else {
     strHtml += '<span title="Page ' + i + '" onclick="javascript:' + this.name + '.toPage(' + i + ');"><a href="javascript:;">' + i + '</a></span>';
    }
   }
   if (this.pageCount >= startPage + 10) strHtml += '<span title="Next 10 Pages" onclick="javascript:' + this.name + '.toPage(' + (startPage + 10) + ');"><a href="javascript:;">...</a></span>';
   if (nextPage > this.pageCount) {
    strHtml += '<span title="Next Page">下一页</span>';
    strHtml += '<span title="Last Page">末页</span>';
   } else {
...
var misc = {
	...
	* @method getParam
	* @param {String} key 
	* @param {String{ type 参数的类型，‘hash’或者‘search’
	* @return {String} 返回key对应的URL参数的值
	**/
	...
	/**
	* 获取当前页面的id，通过获取url最后一个字符串，如:/event/page/53b4bc102514972e28377c34
	*/
	getId : function(){
		if(this.id){
			return this.id;
    ...
    */
	...
	// 快速查找数组里面含有某关键词的集合
	getSmallArrFromBigArrByKeyword: function(arr,keyword,property){
		...
		while(i<l) {
			obj=arr[i];
			temp=property?obj[property]:obj;
	...
	getAge: function(birthday){
	  var arr = [];
	  if(birthday){
	    var milliseconds = new Date() - new Date(birthday);
	    arr = [parseInt(milliseconds/1000/60/60/24/365)];
	  }
	  return  arr.join('');
	},
...
misc.func._user.login=function(param,succ_cb,fail_cb){
	misc.ajax.cdPost(misc.api._user.login, JSON.stringify(param), function(res){
        succ_cb && succ_cb(res);
    }, function(err){
        fail_cb && fail_cb(err);
    });
...
misc.func.admin.get_admin=function(param,succ_cb,fail_cb){
	misc.ajax.cdPost(misc.api.admin.get_admin, JSON.stringify(param), function(res){
        succ_cb && succ_cb(res);
    }, function(err){
        fail_cb && fail_cb(err);
    });
};
misc.func.admin.update_admin=function(param,succ_cb,fail_cb){
	misc.ajax.cdPost(misc.api.admin.update_admin, JSON.stringify(param), function(res){
        succ_cb && succ_cb(res);
    }, function(err){
        fail_cb && fail_cb(err);
    });
};
misc.func.admin.get_admins=function(param,succ_cb,fail_cb){
	misc.ajax.cdPost(misc.api.admin.get_admins, JSON.stringify(param), function(res){
        succ_cb && succ_cb(res);
    }, function(err){
        fail_cb && fail_cb(err);
    });
};
misc.func.activity.get_activity=function(param,succ_cb,fail_cb){
    misc.ajax.cdPost(misc.api.activity.get_activity, JSON.stringify(param), function(res){
        succ_cb && succ_cb(res);
    }, function(err){
        fail_cb && fail_cb(err);
    });
};
misc.func.activity.create_activity=function(param,succ_cb,fail_cb){
    misc.ajax.cdPost(misc.api.activity.create_activity, JSON.stringify(param), function(res){
        succ_cb && succ_cb(res);
    }, function(err){
        fail_cb && fail_cb(err);
    });
};
misc.func.activity.update_activity=function(param,succ_cb,fail_cb){
    misc.ajax.cdPost(misc.api.activity.update_activity, JSON.stringify(param), function(res){
        succ_cb && succ_cb(res);
    }, function(err){
        fail_cb && fail_cb(err);
    });
...
misc.func.activity.get_act_registration=function(param,succ_cb,fail_cb){
    misc.ajax.cdPost(misc.api.activity.get_act_registration, JSON.stringify(param), function(res){
        succ_cb && succ_cb(res);
    }, function(err){
        fail_cb && fail_cb(err);
    });
};
misc.func.activity.get_act_registration_count=function(param,succ_cb,fail_cb){
    misc.ajax.cdPost(misc.api.activity.get_act_registration_count, JSON.stringify(param), function(res){
        succ_cb && succ_cb(res);
    }, function(err){
        fail_cb && fail_cb(err);
    });
};
misc.func.activity.create_act_registration=function(param,succ_cb,fail_cb){
    misc.ajax.cdPost(misc.api.activity.create_act_registration, JSON.stringify(param), function(res){
        succ_cb && succ_cb(res);
    }, function(err){
        fail_cb && fail_cb(err);
    });
...
var errorEnum = {
	'-1': {
		'code': '-1',
		'message': 'Error: -1 Network or Service is not okay',
		'msg': '网络不好，请稍后再试！'
	},
	'202': {
		'code': '202',
		'message': 'Error: 202 Username has already been taken',
		'msg': '该账号已被注册'
	},
	'210': {
		'code': '210',
		'message': 'Error: 210 The username and password mismatch',
		'msg': '密码错误'
	},
	'211': {
		'code': '211',
		'message': 'Error: 211 Could not find user',
		'msg': '该账号不存在'
	}
};
...
/* Custom defined */
function waitRedirect(url,time) {
	time = time ? time : 800;
	setTimeout(function() {
		window.location.href = url;
	},time);
}
function logInBtnText(type){
	if(type == 1){
		$btn_login.text('登录中...');
	} else{
		$btn_login.text('登录');
...
userObj.init = function(checkLogin,initCommon){
	$('title').html('一卡通运营后台');
	userObj.userInfo=userObj.currentUser=JSON.parse(misc.getCookie('userInfo'));
	checkLogin && userObj[checkLogin]();
	userObj.initNavBar();
	userObj.initSideBarMenu(location.hash,location.pathname);
	userObj.initFooter();
	initCommon && userObj[initCommon]();
...
userObj.checkLogin = function(){
	...
	if(!userObj.currentUser){
		...
		userObj.currentUser.get=function(prop){
			return userObj.currentUser[prop];
		};
		$body.show();
	}
};
userObj.register = function(params){
...
userObj.initNavBar = function(){
	var arr = [
	'<div class="container-fluid">',
        '<a class="brand" href="dashboard.html">&nbsp;&nbsp;一卡通运营后台管理系统</a>', 
        ...
        '<ul class="nav pull-right">',
          '<li class="dropdown user" style="margin-top:5px;">',
            '<a href="#" class="dropdown-toggle" data-toggle="dropdown"><span class="username j_username"></span><i class="icon-angle-down"></i></a>',
            '<ul class="dropdown-menu">',
              // '<li><a href="userprofile.html"><i class="icon-user"></i>个人中心</a></li>', 
              '<li><a href="javascipt:;" class="j_logout"> <i class="icon-key"></i>退出</a></li>',
            '</ul>',
          '</li>',
        '</ul>', 
    '</div>'
	...
	$('.j_navbar').html(arr.join(''));
};
userObj.initFooter = function(){
	var arr = [
	'<div class="footer-inner"> 2016-2017 &copy; 一卡通</div>',
    '<div class="footer-tools"><span class="go-top"><i class="icon-angle-up"></i></span></div>'
	];
	$('.j_footer').html(arr.join(''));
};
userObj.initSideBarMenu = function(hash,path){
	var arr = [
	'<li style="margin: 14px 0;">',
	    '<div class="sidebar-toggler hidden-phone"></div>',
	'</li>',
	'<li class="start ',(!hash ? "active":""),' ">',
	     ...
	     '</a>',
	'</li>',
	'<li class="',(hash.indexOf("user")>0 ? "active":""),' j_SuperAdmin_see">',
	    '<a href="javascript:;">',
	        '<i class="icon-user"></i>',
	    ...
	    '<ul class="sub-menu">',
	        ...
	        // '<li class="',(path.indexOf("admin")>0 ? "active":""),'"><a href="admins.html#user"></a></li>',
	        // '<li class="',(path.indexOf("douamap")>0 ? "active":""),'"><a href="douamap.html#user">处理单/居关联</a></li>',
	        // '<li class="',(path.indexOf("docheckin")>0 ? "active":""),'"><a href="docheckin.html#user">处理报到</a></li>',
	    '</ul>',
	'</li>',
	'<li class="',(hash.indexOf("user")>0 ? "active":""),' j_Admins_see">',
	    '<a href="javascript:;">',
	        '<i class="icon-th"></i>',
	        '<span class="title">单位/居委管理面板</span>',
	        '<span class="arrow"></span>',
	    '</a>',
	    '<ul class="sub-menu">',
...
userObj.initData = function(){
	var userRole = userObj.currentUser.get('userRole');
	$('.j_username').text(userObj.roleEnum[userRole].desc + ' ' +userObj.currentUser.username);
...
userObj.initEvent = function(){
    $body.delegate('.j_logout','click',function() {
        userObj.logOut();
    });
};
userObj.logOut = function(){
    AV.User.logOut();
    waitRedirect('login.html', 500);
};
userObj.logIn = function(userName,userPwd){
...
userObj.loadByPID = function(cname,pid,cb,type){
